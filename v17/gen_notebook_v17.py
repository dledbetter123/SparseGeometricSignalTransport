"""Generate architecture_v17.ipynb - Precision-Routed Gaussian Constellations.

V17: Gaussian cloud precision IS the routing mechanism. High-precision modes form
selective connections. Position encoded as learned precision pattern, not phase rotation.
Precision-weighted q/k/v in the matrix fiber. Learned variance evolution."""
import json
import os
...
# ═══════════════════════════════════════════════════════════════
md("""# V17: Precision-Routed Gaussian Constellations
...
The matrix fiber S[t] = Z·S[t-1] + k·v^T stores a dxd matrix per subbundle = 17x17 = 289
values per subbundle x 8 = 2,312 values. Each subbundle can store multiple key-value
...
class V17Config:
    ...
    # Matrix fiber
    n_state_heads: int = 16           # independent matrix-fiber heads
    fiber_key_dim: int = 8            # state matrix dxd per head (16x8x8 = 1024 values)
...
cfg = V17Config(vocab_size=vocab_size)
print(f"V17: vocab={cfg.vocab_size:,} seq={cfg.seq_len} batch={cfg.batch_size}")
print(f"Subbundles: {cfg.n_subbundles}, Modes: {cfg.n_modes}")
print(f"Matrix fiber: {cfg.n_subbundles} x ({cfg.fiber_key_dim}x{cfg.fiber_key_dim}) state")
...
class ConstellationEmbedding(nn.Module):
    \"\"\"Token + position -> Gaussian cloud constellation.
    Variance is a function of BOTH token content and position.
    The precision pattern (which modes are tight) encodes positional identity.\"\"\"
    def __init__(self, cfg):
        ...
        # Positional variance: learned per-position precision pattern
        # This replaces phase-shift as the primary positional encoding
        self.pos_var = nn.Embedding(cfg.max_seq_len, M)
        nn.init.zeros_(self.pos_var.weight)  # starts uniform (no positional preference)

        # Content-position interaction: how token identity modulates positional precision
        self.var_mix = nn.Linear(2 * M, M)
        nn.init.zeros_(self.var_mix.weight)
        nn.init.zeros_(self.var_mix.bias)

        # Keep phase shift as supplementary (it's cheap and provides a smooth baseline)
        freqs = torch.zeros(M)
    ...
    def forward(self, token_ids):
        ...
        # Phase shift positional encoding (supplementary)
        phase = phase + (pos.float().unsqueeze(-1) * self.freqs).unsqueeze(0)

        # Positional variance: which modes are tight at this position
        pos_v = self.pos_var(pos).unsqueeze(0).expand(B, -1, -1)  # (B, T, M)

        # Content-position interaction: token identity modulates the positional pattern
        log_var = self.var_mix(torch.cat([mag, pos_v], dim=-1))  # (B, T, M)
...
The matrix fiber stores a dxd matrix per subbundle: S[t] = Z·S[t-1] + k·v^T.
...
code("""def matrix_parallel_scan(A_diag, B_mat):
    \"\"\"Parallel scan for: S[t] = a[t] * S[t-1] + B[t]
    where a[t] is a scalar decay and B[t] is a dxd deposit matrix.
    A_diag: (N, T) scalar decays. B_mat: (N, T, d, d) deposit matrices.
    Returns: (N, T, d, d) accumulated states. O(T log T) parallel.\"\"\"
    N, T, d, _ = B_mat.shape
    a = A_diag
    b = B_mat
...
class MatrixFiber(nn.Module):
    \"\"\"Precision-routed parallel matrix fiber. 16 heads x 8x8 state matrices.

    The key change from V16e: Gaussian precision gates what enters the state.
    High-precision modes deposit and query strongly. Low-precision modes are silent.
    This creates selective, sparse routing — tokens only communicate through
    modes where BOTH have high precision (overlapping tight cones).\"\"\"
    def __init__(self, cfg):
        super().__init__()
        M = cfg.n_modes
        d = cfg.fiber_key_dim         # 8
        H = cfg.n_state_heads         # 16

        # Project precision-weighted signal to q, k, v, decay
        # Input: effective_mag (M) + phase (M) + precision (M) = 3M
        self.proj = nn.Linear(3 * M, H * (3 * d + 1))
        self.out_proj = nn.Linear(H * d, M)
        self.H = H
        self.d = d
    ...
    def forward(self, constellation):
        ...
        # PRECISION-WEIGHTED INPUT: only confident modes contribute
        precision = constellation.precision()              # exp(-log_var): (B, T, M)
        prec_gate = torch.sigmoid(precision - 1.0)        # soft gate centered at precision=1
        effective_mag = constellation.mag * prec_gate       # confident modes pass, uncertain attenuated

        # Project from precision-weighted features
        inp = torch.cat([effective_mag, constellation.phase, prec_gate], dim=-1)  # (B, T, 3M)
        proj = self.proj(inp).reshape(B, T, H, 3 * d + 1)

        q = F.elu(proj[..., :d]) + 1
        k = F.elu(proj[..., d:2*d]) + 1
        v = proj[..., 2*d:3*d]
        gamma = torch.sigmoid(proj[..., 3*d:]).squeeze(-1)  # (B, T, H)

        # Deposit matrices: k outer v
...
class VarianceUpdate(nn.Module):
    \"\"\"Learned variance evolution. Replaces fixed -0.1 per block.
    The variance update depends on the fiber messages (what context was retrieved)
    and the current precision (how confident the token already is).
    The model learns: which modes to tighten (increase confidence) and which to
    loosen (decrease confidence) based on what it learned from context.\"\"\"
    def __init__(self, cfg):
        super().__init__()
        M = cfg.n_modes
        self.net = nn.Sequential(
            nn.Linear(2 * M, M),  # input: messages + current log_var
            nn.Tanh(),            # bounded output: delta in (-1, 1)
        )
        nn.init.zeros_(self.net[0].weight)
        nn.init.zeros_(self.net[0].bias)

    def forward(self, log_var, messages):
        delta = self.net(torch.cat([messages, log_var], dim=-1))
        new_log_var = log_var + 0.1 * delta  # small learned adjustments
        return new_log_var.clamp(min=-6, max=2)


class V17Block(nn.Module):
    \"\"\"Precision-routed matrix fiber + Parseval filter + irfft + local conv + FFN.
    Variance evolves through learned update, not fixed decay.\"\"\"
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
    ...
    def forward(self, constellation):
        ...
        # --- Precision-routed matrix fiber → messages ---
        ...
        # --- Learned variance evolution ---
        new_log_var = self.var_update(constellation.log_var, messages)

        return Constellation(new_mag, new_phase, new_log_var)


class V17Model(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        self.embedding = ConstellationEmbedding(cfg)
        self.blocks = nn.ModuleList([V17Block(cfg) for _ in range(cfg.n_blocks)])
...
_b = V17Block(cfg)
print(f"V17Block: {count_params(_b):,} params")
...
models = {}
models['V17'] = V17Model(cfg).to(device)
...
code("""colors = {'V17': 'tab:blue', 'GPT-224d': 'black'}
fig, axes = plt.subplots(2, 3, figsize=(20, 10))
fig.suptitle('V17 (Precision-Routed) vs GPT-224d on WikiText-103', fontsize=14, fontweight='bold')
ax = axes[0, 0]
...
outpath = os.path.join(os.path.dirname(os.path.abspath(__file__)), "architecture_v17.ipynb")
